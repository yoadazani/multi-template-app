...
import { userData } from '../data/__tests__/userData';
...
describe('Given authController', () => {
    ...
    describe('Given signup route', () => {
        ...
        it('Should return status 403 and error message and property', async () => {
            ...
            newUsers.forEach((newUserData) => {
                Promise.all([
                    request.post('/api/v1/auth/signup').send(newUserData),
                ]).then(([response]) => {
        ...
        it('Should return status 409 and error message and property', async () => {
            ...
            prismaMock.user.findFirst.mockResolvedValue(userData as UserType);
    ...
    describe('Given login route', () => {
        it('should return status 200 and user data', async () => {
            ...
            const res = await request.get('/api/v1/auth/login').send(req);
        ...
        it('should return status 403 and error message and property', async () => {
            ...
            newUsers.forEach((newUserData) => {
                Promise.all([
                    request.get('/api/v1/auth/login').send(newUserData),
                ]).then(([response]) => {
                    expect(response.status).toBe(403);
    ...
    describe('Given me route', () => {
        it('should return status 200 and user data', async () => {
            prismaMock.user.findUnique.mockResolvedValue(userData as UserType);
